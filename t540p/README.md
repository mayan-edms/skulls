# Skulls - [Thinkpad T540p](https://pcsupport.lenovo.com/uu/de/products/laptops-and-netbooks/thinkpad-t-series-laptops/thinkpad-t540p)
...
## TL;DR
1. run `sudo ./skulls.sh -b t540p` on your current T540p Linux system
...
and `sudo ./external_install_top.sh -b t540p` on the top chip of the two.
3. For updating later, run `./skulls.sh -b t540p`. No need to disassemble.
...
#### before you begin
Run Linux on your T540p, install `dmidecode` and run
`sudo ./skulls.sh -b t540p`. It simply prints system information and
...
This is the right chip of the 2. Choose the image to flash during running:
    ...
    sudo ./external_install_top.sh -b t540p -k <backup-file-to-create>


This selects and flashes it and that's it.
Keep the backup safe, assemble and
turn on the T540p. coreboot will do hardware init and start SeaBIOS.

## Updating
If you have locked your flash (i.e. `./external_install_bottom -l`) you can
flash externally using `external_install_top.sh -b t540p` just like the
first time, see above. Only the "upper" 4MB chip has to be written.

It is recommended to do the update directly on your T540p using Linux
though. This is considered more safe for your hardware and is very convenient -
just install the "flashrom" program and run  `./skulls.sh -b t540p`, see below.

1. boot Linux with the `iomem=relaxed` boot parameter (for example in /etc/default/grub `GRUB_CMDLINE_LINUX_DEFAULT`)
2. [download](https://github.com/merge/skulls/releases) the latest Skulls release tarball and unpack it or check for updates by running `./skulls.sh -b t540p -U`.
3. run `sudo ./skulls.sh -b t540p` and choose the image to flash.
...
## Why does this work?
On the T540p, there are 2 physical "BIOS" chips. The 4MB
